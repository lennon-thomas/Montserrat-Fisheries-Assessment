...
# FinalBio=c(0.25,0.5)
  ...
  for(c in 1:CatchJitters) {
    ...
    if(length(r1)>=10) {
      ...
      Output<- as.data.frame(matrix(NA,nrow=length(yr),ncol=10))
      
      colnames(Output)<- c('Year','Method','SampleSize','Value','LowerCI','UpperCI','SD','Metric','Flag','Species')
      ...
      MCDetails<- as.data.frame(matrix(NA,nrow=length(yr),ncol=10))
      MCDetails$Year<- yr
